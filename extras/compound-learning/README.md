...
```powershell
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/Assure-DeFi/mason/main/extras/compound-learning/install.ps1" -OutFile "install-patterns.ps1"; .\install-patterns.ps1; Remove-Item "install-patterns.ps1"
```
...
The installer adds this section to your `mason.config.json`:
...
Then remove the `patternLearning` section from `mason.config.json`.
