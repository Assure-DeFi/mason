...
## Compound Driver: The FIFTH Self-Feeding Signal — The Transcript Check Excluded Only ONE of Two Sibling Automated Drivers

**Discovered**: 2026-07-24
**Context**: The 07-22 dirty-tree receipt fix worked exactly as predicted — the 07-24 01:15 run logged `dirty tree unchanged since last full review (9b0e4fb0…); not a signal`. But mason still did NOT skip: it fired on `signal: session transcript 071ed27b…jsonl touched in last 25h`. That transcript is `entrypoint=sdk-cli` / `promptSource=sdk`, mtime 07-23 03:01 — the AUTOMATED 03:00 `auto-compound.sh` execute-approved run (`claude -p "Use the /execute-approved skill…"`), not a human session. Signal 5 (transcript-touched) excluded only transcripts containing this driver's OWN compound prompt string (`"Extract any learnings that weren't already compounded at session end"`), so the sibling execute-approved launcher's transcript was never excluded and re-fired every night (25h window always covers the prior night's 03:00 run).
**Pattern**: When a gate excludes "its own runs" by matching a per-launcher PROMPT STRING, it fails open on every OTHER automated launcher — the enumerate-known-strings denylist class. Fix structurally: ALL headless `claude -p` runs (this driver's compound pass, `auto-compound.sh`'s execute-approved pass, and any future one) write transcripts marked `"entrypoint":"sdk-cli"` (also `"promptSource":"sdk"`); a real interactive terminal session is `entrypoint=cli` and never `sdk-cli`. Changed the exclusion in `daily-compound-review.sh` signal 5 from the compound-prompt grep to `grep -qF '"entrypoint":"sdk-cli"'`, closing the whole class at once. Applied via atomic `mv` (driver PID was live). Verified: both automated transcripts now excluded, a fabricated `entrypoint=cli` transcript still fires.
**Why**: Fifth distinct self-feeding signal in the same gate, same root cause as the prior four — a driver that both measures a signal and produces the artifact that satisfies it. The prompt-string exclusion was itself the fails-open bug: it "knew" one launcher and was blind to its sibling. A structural marker (headless vs interactive) covers every launcher, present and future, where an enumerated string list covers only the ones you remembered. Mason should now genuinely skip on idle nights (only automated transcripts exist); if it STILL fires, read the log line for a sixth signal — don't assume the fix failed.
