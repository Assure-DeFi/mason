...
export async function executeRemotely(
  ...
  try {
    ...
    // Track file index for checkpoint numbering
    let fileIndex = 0;
    ...
    // Update total checkpoints now that we know actual file count
    const actualTotalCheckpoints = calculateTotalCheckpoints(
      allFileChanges.length,
    );
    for (const item of successfulItems) {
      await setTotalCheckpoints(supabase, item.id, actualTotalCheckpoints);
    }
    ...
    // Create execution_progress records for ExecutionStatusModal visualization
    ...
    // Checkpoint: Validation phase (TypeScript, ESLint, Build)
    for (const item of successfulItems) {
      await updateCheckpoint(
        supabase,
        item.id,
        CHECKPOINT_CATALOG.TYPESCRIPT_CHECK,
        {
          phase: 'inspection',
        },
      );
    ...
    // Checkpoint: Committing changes
    for (const item of successfulItems) {
      await updateCheckpoint(supabase, item.id, CHECKPOINT_CATALOG.COMMITTING);
    }
    ...
    for (const item of successfulItems) {
      await updateCheckpoint(supabase, item.id, CHECKPOINT_CATALOG.CREATING_PR);
    }
...
*Please review the changes carefully before merging.*`;
    ...
    // Checkpoint: Complete - emit final checkpoint
    for (const item of successfulItems) {
      await updateCheckpoint(supabase, item.id, CHECKPOINT_CATALOG.COMPLETE, {
        phase: 'complete',
      });
    }

    // Mark all successful items as complete (legacy + new)
    ...
    if (completionFailures.length > 0) {
      ...
      await log(
        runId,
        'info',
        `All ${successfulItems.length} items marked complete in ExecutionStatusModal`,
      );
    }
    ...
    try {
      ...
      // Mark progress records as failed for visibility in ExecutionStatusModal
      await Promise.allSettled(
