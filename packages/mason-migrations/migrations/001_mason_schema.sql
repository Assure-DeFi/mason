...
--------------------------------------------------------------------------------
-- Table: mason_pm_execution_tasks
...
-- mason_pm_backlog_items indexes
...
-- mason_pm_execution_tasks indexes
...
-- Trigger to update updated_at timestamp on mason_pm_backlog_items
...
CREATE POLICY "Allow all access to mason_pm_execution_runs" ON mason_pm_execution_runs
  FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all access to mason_pm_execution_tasks" ON mason_pm_execution_tasks
  FOR ALL USING (true) WITH CHECK (true);
...
COMMENT ON TABLE mason_pm_execution_runs IS 'Tracks execution runs when approved items are implemented';
COMMENT ON TABLE mason_pm_execution_tasks IS 'Individual tasks within an execution run';

COMMENT ON COLUMN mason_pm_backlog_items.priority_score IS 'Auto-calculated: (impact_score * 2) - effort_score';
COMMENT ON COLUMN mason_pm_backlog_items.benefits IS 'JSON array of benefit objects with category, icon, title, description';
COMMENT ON COLUMN mason_pm_backlog_items.complexity IS 'Complexity rating 1-5 (1=trivial, 5=very complex)';
COMMENT ON COLUMN mason_pm_backlog_items.type IS 'Type category: dashboard, discovery, auth, backend';
COMMENT ON COLUMN mason_pm_backlog_items.area IS 'Area: frontend or backend';

COMMENT ON COLUMN mason_pm_execution_runs.item_count IS 'Number of backlog items included in this run';
COMMENT ON COLUMN mason_pm_execution_runs.tasks_completed IS 'Count of successfully completed tasks';
COMMENT ON COLUMN mason_pm_execution_runs.tasks_failed IS 'Count of failed tasks';

COMMENT ON COLUMN mason_pm_execution_tasks.wave_number IS 'Wave number for parallel execution (1, 2, 3...)';
COMMENT ON COLUMN mason_pm_execution_tasks.task_number IS 'Task number within wave';
COMMENT ON COLUMN mason_pm_execution_tasks.subagent_type IS 'Type of Claude Code subagent to use';
COMMENT ON COLUMN mason_pm_execution_tasks.result_summary IS 'Brief summary of task result for debugging';

COMMENT ON VIEW mason_pm_execution_tasks_detail IS 'Tasks with joined item and run information';
