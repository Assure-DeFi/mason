...
CREATE TABLE IF NOT EXISTS mason_users (
  ...
  -- The service_role_key is encrypted at the application level
  supabase_url TEXT,
...
CREATE OR REPLACE FUNCTION update_mason_users_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trigger_mason_users_updated_at ON mason_users;
CREATE TRIGGER trigger_mason_users_updated_at
  BEFORE UPDATE ON mason_users
  FOR EACH ROW
  EXECUTE FUNCTION update_mason_users_updated_at();

--------------------------------------------------------------------------------
-- Table: mason_github_repositories
...
-- Trigger to update updated_at
CREATE OR REPLACE FUNCTION update_mason_github_repositories_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trigger_mason_github_repositories_updated_at ON mason_github_repositories;
CREATE TRIGGER trigger_mason_github_repositories_updated_at
  BEFORE UPDATE ON mason_github_repositories
  FOR EACH ROW
  EXECUTE FUNCTION update_mason_github_repositories_updated_at();

--------------------------------------------------------------------------------
-- Table: mason_remote_execution_runs
...
--------------------------------------------------------------------------------
-- Modify mason_pm_backlog_items: Add user_id and repository_id
--------------------------------------------------------------------------------
...
-- mason_users indexes
CREATE INDEX IF NOT EXISTS idx_mason_users_github_id ON mason_users(github_id);
CREATE INDEX IF NOT EXISTS idx_mason_users_github_username ON mason_users(github_username);
CREATE INDEX IF NOT EXISTS idx_mason_users_is_active ON mason_users(is_active);

-- mason_github_repositories indexes
CREATE INDEX IF NOT EXISTS idx_mason_github_repositories_user_id ON mason_github_repositories(user_id);
CREATE INDEX IF NOT EXISTS idx_mason_github_repositories_github_repo_id ON mason_github_repositories(github_repo_id);
CREATE INDEX IF NOT EXISTS idx_mason_github_repositories_full_name ON mason_github_repositories(github_full_name);
CREATE INDEX IF NOT EXISTS idx_mason_github_repositories_is_active ON mason_github_repositories(is_active);

-- mason_remote_execution_runs indexes
CREATE INDEX IF NOT EXISTS idx_mason_remote_execution_runs_user_id ON mason_remote_execution_runs(user_id);
CREATE INDEX IF NOT EXISTS idx_mason_remote_execution_runs_repository_id ON mason_remote_execution_runs(repository_id);
CREATE INDEX IF NOT EXISTS idx_mason_remote_execution_runs_status ON mason_remote_execution_runs(status);
CREATE INDEX IF NOT EXISTS idx_mason_remote_execution_runs_created_at ON mason_remote_execution_runs(created_at DESC);

-- mason_execution_logs indexes
...
-- mason_pm_backlog_items new indexes
...
CREATE POLICY "Users can manage own repositories" ON mason_github_repositories
...
CREATE POLICY "Users can manage own execution runs" ON mason_remote_execution_runs
  FOR ALL USING (true) WITH CHECK (true);

-- Execution logs: Users can view logs for their runs
CREATE POLICY "Users can view execution logs" ON mason_execution_logs
  FOR SELECT USING (true);

CREATE POLICY "System can insert execution logs" ON mason_execution_logs
...
COMMENT ON TABLE mason_users IS 'Authenticated users with GitHub OAuth tokens';
COMMENT ON TABLE mason_github_repositories IS 'Connected GitHub repositories per user';
COMMENT ON TABLE mason_remote_execution_runs IS 'Dashboard-initiated execution runs';
COMMENT ON TABLE mason_execution_logs IS 'Real-time execution logs for progress streaming';

COMMENT ON COLUMN mason_users.github_access_token IS 'GitHub OAuth access token (encrypted at rest by Supabase)';
COMMENT ON COLUMN mason_users.default_repository_id IS 'User preferred default repository';
...
COMMENT ON COLUMN mason_github_repositories.github_full_name IS 'Full repository name in owner/repo format';
COMMENT ON COLUMN mason_github_repositories.last_synced_at IS 'Last time repo metadata was synced from GitHub';

COMMENT ON COLUMN mason_remote_execution_runs.item_ids IS 'Array of backlog item UUIDs being executed';
COMMENT ON COLUMN mason_remote_execution_runs.branch_name IS 'Git branch created for this execution';
COMMENT ON COLUMN mason_remote_execution_runs.base_branch IS 'Base branch to create PR against';

COMMENT ON COLUMN mason_execution_logs.metadata IS 'Optional JSON metadata (file paths, error details, etc.)';

COMMENT ON COLUMN mason_pm_backlog_items.user_id IS 'Owner of this backlog item (nullable for CLI-created items)';
COMMENT ON COLUMN mason_pm_backlog_items.repository_id IS 'Target repository for this item (nullable for CLI-created items)';
